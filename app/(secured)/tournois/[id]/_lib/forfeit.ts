 ...
 *
 * L'abandon n'a de sens que dans un tournoi Survie en cours : les formats à
 * élimination n'ont pas de notion d'abandon en cours de route (le match perdu
 * suffit). Un représentant de l'équipe déclare son propre forfait ; l'arbitrage
 * peut le déclarer pour n'importe quelle équipe. Même règle côté serveur, dans
 * `app/api/tournaments/[id]/forfeit/route.ts`.
 *
...
export function canForfeitTeam(context: ForfeitContext, teamId: number): boolean {
  if (context.format !== "SURVIVAL" || context.state !== "RUNNING") return false;
