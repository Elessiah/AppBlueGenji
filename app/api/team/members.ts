...
export async function members(body: {id_user: number | undefined,
                                  id_team: number | undefined},
                              id_user: number,
                              is_remove: boolean = false) : Promise<NextResponse> {
    if (body.id_user === undefined || body.id_team === undefined)
    ...
    let status: status;
    if (is_remove)
        status = await database.rmTeamMember(body.id_user);
    else
        status = await database.addTeamMember(body.id_user, body.id_team);
    if (!status.success)
