...
import { POST } from "@/app/api/benevoles/photo/route";
...
function fileReq(file?: File) {
  ...
  return new Request("http://localhost/api/benevoles/photo", { method: "POST", body: form });
}
...
describe("POST /api/benevoles/photo", () => {
  ...
  it("stores the photo and returns its served url for admins", async () => {
    (getCurrentUser as jest.Mock).mockResolvedValue(admin as never);
    (processAndStoreImage as jest.Mock).mockResolvedValue("/uploads/benevoles/1-abc.webp" as never);
