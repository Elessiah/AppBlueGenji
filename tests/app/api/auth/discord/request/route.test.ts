...
describe("POST /api/auth/discord/request", () => {
  ...
  it("maps BOT_INTERNAL_TIMEOUT to 504", async () => {
    resolveDiscordUserMock.mockRejectedValue(new Error("BOT_INTERNAL_TIMEOUT"));

    const response = await POST(buildRequest({ discordId: "keryan" }));
    const payload = (await response.json()) as { error: string };

    expect(response.status).toBe(504);
    expect(payload.error).toBe("BOT_INTERNAL_TIMEOUT");
    expect(sendDiscordLoginCodeMock).not.toHaveBeenCalled();
  });
