...
describe("listPlayers visibility", () => {
  ...
  it("masks pseudo and avatar of other players when hidden", async () => {
    const players = await runList(
      [userRow({ visible_pseudo: 0, visible_avatar: 0 })],
      999,
    );
    expect(players[0].pseudo).toBe("Joueur #7");
    expect(players[0].avatarUrl).toBeNull();
  });

  it("keeps pseudo and avatar visible when the flags are set", async () => {
