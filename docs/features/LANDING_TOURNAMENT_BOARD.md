...
- **État** — l'étape vient de `computeTournamentProgress` (la frise de la fiche),
  ...
  `RUNNING` → « En cours ».
- **Action** — elle mène toujours à la fiche ; seul le libellé change :
  « S'inscrire » aux inscriptions ouvertes avec une place libre, « Voir le
  bracket » sur un tournoi lancé en élimination simple ou double, « Suivre le
  tournoi » sur un tournoi lancé sans arbre (Suisse, Survie, BlueGenji Survie,
  multi-phases), « Voir le tournoi » sinon.
