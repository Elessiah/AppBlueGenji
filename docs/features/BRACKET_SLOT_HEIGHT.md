...
La hauteur de créneau n'est plus une constante mais la **plus haute carte
réellement rendue**, mesurée dans le navigateur :

```
hauteur = max(plancher, ⌈carte la plus haute⌉ + air)
```
...
La hauteur reste **uniforme** sur toute la section. Un créneau par carte serait
plus compact, mais les traits de liaison sont posés à un multiple de la hauteur :
des créneaux inégaux demanderaient une position cumulée par match *et* par round,
et deux rounds voisins ne s'aligneraient plus.
