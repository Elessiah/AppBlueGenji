...
Deux **paliers de fraîcheur** (`lib/shared/refresh-tiers.ts`, pur, source unique des cadences) : `PRIORITY` (staff `tournaments` + engagés du tournoi) reçoit dans la seconde, `STANDARD` (spectateurs) par fenêtres plus larges. Le palier est décidé par le serveur à la connexion, jamais déclaré par le client.
