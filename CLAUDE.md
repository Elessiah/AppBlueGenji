...
Côté charge : cache mémoire **à vol unique** (`lib/server/cache.ts`) sur l'instantané, la liste publique et les agrégats de la vitrine — cent appels concurrents partagent un seul calcul ; invalidation centralisée dans `tournaments/notifications.ts` ; plafonds de débit larges (`lib/server/rate-limit.ts` + `api-guard.ts`) ; étranglement de `syncVisibleTournaments` porté de 1 s à 15 s. Voir `docs/features/REALTIME_REFRESH.md`.
